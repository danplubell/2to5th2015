...
site :: Snap ()
site = route [ ("location", locationHandler)] <|> defaultHandler
defaultHandler::Snap()
...
locationHandler = do
  ...
  case (latparam,lonparam) of
    ...
    (Just la,Just lo) -> do tze <- liftIO $ handleLocation la lo
                            case tze  of
                              Left err    -> sendBadRequest err
                              Right tzstr -> sendBackTZ tzstr
  return ()
  where sendBackTZ tz = writeBS $ BL.toStrict $ encode tz
sendBadRequest::String -> Snap()
sendBadRequest s = do modifyResponse $ setResponseStatus 400 "Bad Request"
